...
def create_new_page(conn, site_id, page_type_code, url, html_content, html_content_hash, http_status_code, accessed_time):
    ...
    query = f"UPDATE crawldb.page SET site_id = %s, page_type_code = %s, html_content = %s, html_content_hash = %s, http_status_code = %s, accessed_time = current_timestamp WHERE url = %s RETURNING crawldb.page.id"
    cur.execute(query, (site_id, page_type_code, html_content, html_content_hash, http_status_code, url))
